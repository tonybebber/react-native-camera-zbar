...
public class RCTCameraOld {
    private static RCTCameraOld ourInstance;
    ...
    public static RCTCameraOld getInstance() {
        return ourInstance;
    }
    public static void createInstance(int deviceOrientation) {
        ourInstance = new RCTCameraOld(deviceOrientation);
    }
    ...
    public synchronized Camera acquireCameraInstance(int type) {
        if (null == _cameras.get(type) && null != _cameraTypeToIndex.get(type)) {
            try {
                ...
                Log.e("RCTCameraOld", "acquireCameraInstance failed", e);
            }
        }
    ...
    public void setOrientation(int orientation) {
        ...
        adjustPreviewLayout(RCTCameraOldModule.RCT_CAMERA_TYPE_FRONT);
        adjustPreviewLayout(RCTCameraOldModule.RCT_CAMERA_TYPE_BACK);
    }
    ...
    public boolean isZoomEnabled() { return _zoomEnabled; }
    ...
    public void setCaptureMode(final int cameraType, final int captureMode) {
        ...
        parameters.setRecordingHint(captureMode == RCTCameraOldModule.RCT_CAMERA_CAPTURE_MODE_VIDEO);
    ...
    public void setCaptureQuality(int cameraType, String captureQuality) {
        ...
        switch (captureQuality) {
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_LOW:
                pictureSize = getSmallestSize(supportedSizes);
                break;
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_MEDIUM:
                pictureSize = supportedSizes.get(supportedSizes.size() / 2);
                break;
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_HIGH:
                pictureSize = getBestSize(parameters.getSupportedPictureSizes(), Integer.MAX_VALUE, Integer.MAX_VALUE);
                break;
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_PREVIEW:
                ...
                break;
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_480P:
                pictureSize = getBestSize(supportedSizes, RESOLUTION_480P.width, RESOLUTION_480P.height);
                break;
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_720P:
                pictureSize = getBestSize(supportedSizes, RESOLUTION_720P.width, RESOLUTION_720P.height);
                break;
            case RCTCameraOldModule.RCT_CAMERA_CAPTURE_QUALITY_1080P:
    ...
    public void setTorchMode(int cameraType, int torchMode) {
        ...
        switch (torchMode) {
            case RCTCameraOldModule.RCT_CAMERA_TORCH_MODE_ON:
                value = Camera.Parameters.FLASH_MODE_TORCH;
                break;
            case RCTCameraOldModule.RCT_CAMERA_TORCH_MODE_OFF:
    ...
    public void setFlashMode(int cameraType, int flashMode) {
        ...
        String value = parameters.getFlashMode();
        switch (flashMode) {
            case RCTCameraOldModule.RCT_CAMERA_FLASH_MODE_AUTO:
                value = Camera.Parameters.FLASH_MODE_AUTO;
                break;
            case RCTCameraOldModule.RCT_CAMERA_FLASH_MODE_ON:
                value = Camera.Parameters.FLASH_MODE_ON;
                break;
            case RCTCameraOldModule.RCT_CAMERA_FLASH_MODE_OFF:
                value = Camera.Parameters.FLASH_MODE_OFF;
                break;
        }
        List<String> flashModes = parameters.getSupportedFlashModes();
    ...
    private RCTCameraOld(int deviceOrientation) {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT && _cameraInfos.get(RCTCameraOldModule.RCT_CAMERA_TYPE_FRONT) == null) {
                _cameraInfos.put(RCTCameraOldModule.RCT_CAMERA_TYPE_FRONT, new CameraInfoWrapper(info));
                _cameraTypeToIndex.put(RCTCameraOldModule.RCT_CAMERA_TYPE_FRONT, i);
                acquireCameraInstance(RCTCameraOldModule.RCT_CAMERA_TYPE_FRONT);
                releaseCameraInstance(RCTCameraOldModule.RCT_CAMERA_TYPE_FRONT);
            } else if (info.facing == Camera.CameraInfo.CAMERA_FACING_BACK && _cameraInfos.get(RCTCameraOldModule.RCT_CAMERA_TYPE_BACK) == null) {
                _cameraInfos.put(RCTCameraOldModule.RCT_CAMERA_TYPE_BACK, new CameraInfoWrapper(info));
                _cameraTypeToIndex.put(RCTCameraOldModule.RCT_CAMERA_TYPE_BACK, i);
                acquireCameraInstance(RCTCameraOldModule.RCT_CAMERA_TYPE_BACK);
                releaseCameraInstance(RCTCameraOldModule.RCT_CAMERA_TYPE_BACK);
            }
        }
    }
